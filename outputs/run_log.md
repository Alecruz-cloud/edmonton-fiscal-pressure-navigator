...
### Run 13 — CPI display context shipped; ML forecast refused (2026-07-18)
- **Ran:** owner reviewed the brief's limitations and approved two decisions. (1) Inflation: StatCan 18-10-0005-01 (All-items Alberta CPI, 2002=100) added as approved Source D (`data/raw/18100005-eng.zip`, 1.2 MB); `build_mart.py::extract_cpi` with hard guards (unit, status flags, duplicates, missing years); 208 Edmonton-only `*_real2025` mart rows (960 total), `constant 2025` labels, `display_only_context`; drill-down chart toggle that hides peer series so two dollar bases never share an axis; dollar-basis panel in Methodology; brief and all governance docs updated. (2) Forecasting: **refused and documented** — eight annual observations per metric cannot support a defensible model; the spec now records the governed path (longer approved history, approved method with mandatory uncertainty reporting, own review gate).
- **Good:** 57/57 tests (6 new: pinned CPI re-extraction, base-year identity, deflation known-answer, scope/traces, engine-blindness); validator anchors 8 → 10; signals byte-identical pre/post (engine never reads real rows); browser walk verified toggle on `exp_per_capita` ($3,020.55 nominal 2018 ↔ $3,699.43 constant 2025) and its absence on ratio metrics; new `#view-drill:metric_id` deep-link.
- **Caught by the process:** a from-memory "$3,019.86" for 2018 expense per capita failed verification against the mart ($3,020.55) before publication — load-bearing rule 1 applied to the author. No new skills created: the existing `metric-validation` skill was extended (dollar-basis rule + CPI consistency checks); no ML skill because no ML was built.
- **Submission v4:** drill/method/brief figures re-captured, embedded brief synced, write-up section "Closing the nominal-dollar limitation — and declining to forecast" added, test counts updated.

### Run 14 — Civic identity: Edmonton palette, authorship masthead, source-reference footer (2026-07-18)
- **Ran:** owner-requested format pass modelled on their 10-K-style briefs. Palette sampled **live from edmonton.ca** (not from memory): primary `#005087`, deep navy `#132332`, body grey `#444444`, surface `#F5F4F5`, gold `#FFD400`; applied to tokens, sparklines, chart series, map ramp/rings and legend. The production banner ("LIVE BUILD…") became an **authorship masthead** (Prepared by Alejandro A. Cruz · MMA Candidate, U of A — name in gold on navy); the pipeline footer became a **source-reference footer** (all five sources incl. CPI catalogue number and basemap attribution, plus the basis note: nominal unless constant-2025$, rules v1.2 pointer to Methodology, and "not a City of Edmonton publication").
- **Good:** semantic status colours kept (functional, not brand); provenance not lost — the banner's content moved to footer/stamps/methodology; PRODUCT.md palette section updated with the no-City-marks guard; live DOM verification of tokens, masthead, footer and re-coloured map (19 bubbles, ring `#132332`, ramp to `#005087`); console clean.
- **Bug caught by pixel-checking the captures:** Chromium's fragment scroll cut the masthead out of the `#view-exec` screenshot (the exec section is the only one visible at load, so the anchor scroll fired only there — and re-fired flakily for method/map once tabs became visible). Fixed with a scroll-safe `#!view-…` deep-link variant; verified by counting navy masthead pixels in every capture (43,338 in all five).
- **Submission v5:** all five view figures re-captured under the civic identity; screenshots-intro note explains the format pass and the student-authorship guard.

### Run 15 — Brief approved for publication; rubric compliance audit (2026-07-18)
- **Ran:** the owner reviewed and **approved the management brief** — the last open governance gate. DRAFT markers replaced with the dated approval (owner, 2026-07-18) in `outputs/brief_2025.md`, the dashboard's Brief view (verified chip), and the submission's embedded brief + View 5 caption; the brief figure re-captured with the approval on screen (masthead pixel-verified).
- **Rubric audit (assignment doc, six components):** (1) Opportunity & judgment — named user, named decision, rejected alternative, two documented refusals (neighbourhood map, ML forecast). (2) Living spec — dated revisions across 2026-07-17/18 (map, methodology, CPI, forecasting boundary, dollar-basis rule). (3) Knowledge — GoA workbooks + population connected as read-only live files; three supplementary approved sources with scoped, dated approvals; smallest-context rule. (4) Skills & infrastructure — two skills with blocking code checks (10 anchors), two subagents with boundaries, three deterministic pipelines, 57 tests. (5) Use & evaluation — 15 logged runs, before-and-after arcs (tests 19→57; rules v1.0→v1.2; two critic rounds with owner triage), exercised review gates including this approval. (6) Write-up — full account plus honesty notes (self-caught $3,020.55 correction, coverage caveats). All six deliverable elements present in the single HTML file; optional screen recording not included (explicitly optional).
- **Result:** no gaps found; submission v6 is the release candidate.
